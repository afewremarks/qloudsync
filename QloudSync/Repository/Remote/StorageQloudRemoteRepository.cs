...
{
	public class StorageQloudRemoteRepository : RemoteRepositoryController
	{
        string bucketName;
        public StorageQloudRemoteRepository (LogicalRepositoryController logicalController): base(logicalController)
		{
        ...
        public override TransferResponse Upload (RepoObject file)
        {
        ...
        {
            CurrentTransfer = new TransferResponse(destination, TransferType.UPLOAD);
            if (source.InTrash)
        ...
        public void CopyToTrashFolder (RepoObject source, RepoObject destination)
        {
            GenericCopy (bucketName, source.AbsolutePath, destination.TrashRelativePath, destination.Name);
        }
        
        public void Move (RepoObject source, RepoObject destination)
        {
        ...
        public override TransferResponse MoveFileToTrash (RepoObject  file)
        {
        ...
        public bool IsTrashFile (RepoObject file)
        {
            return file.InTrash;
        }
        
        public override TransferResponse SendLocalVersionToTrash (RepoObject file)
        {
            CurrentTransfer = new TransferResponse(file, TransferType.REMOVE);
            if (!System.IO.File.Exists (file.FullLocalName)) {
                CurrentTransfer.Status = TransferStatus.DONE_WITH_ERROR;
        ...
        public bool SendRemoteVersionToTrash (RepoObject file)
        {
            string destinationName;
            if (file.IsAFolder)
        ...
        public void UpdateTrashFolder (RepoObject  file)
        {
            if (file.IsAFolder)
                return;
            /*
            ...
            List<RepoObject> versions = TrashFiles.Where (tf => !tf.AsS3Object.Key.EndsWith("/") && tf.AbsolutePath != string.Empty && tf.AbsolutePath.Substring(0, tf.AbsolutePath.Length-3)== file.AbsolutePath).OrderByDescending(t => t.AbsolutePath).ToList<RepoObject> ();
            
            int overload = versions.Count-2;
            for (int i=0; i<overload; i++) {
                RepoObject v = versions[i];
                GenericDelete(v.TrashAbsolutePath);
                versions.Remove(v);
            }
            
            foreach (RepoObject version in versions) {
                ...
                RepoObject newVersion = new RepoObject (newName);
        ...
        private List<RepoObject> GetVersionsOrderByLastModified (RepoObject  file)  {
            return null; // TrashFiles.Where (ft => ft.AbsolutePath.Contains (file.AbsolutePath)).OrderBy(ft => ft.AsS3Object.LastModified).ToList<RepoObject>();
        }
        ...
        protected List<RepoObject> GetStorageQloudObjects (List<S3Object> files)
        {
            List <RepoObject> remoteFiles = new List <RepoObject> ();
            
            foreach (S3Object file in files) {
                //remoteFiles.Add (new RepoObject (file.Key));
            }
            return remoteFiles;
        }
        ...
        public override TransferResponse Download (RepoObject  file)
        {        
        ...
        public override TransferResponse CreateFolder (RepoObject folder)
		{
...
//
//                    RepoObject clockFile = new RepoObject (Constant.CLOCK_TIME);
//    				PutObjectRequest putObject = new PutObjectRequest ()
//    				{
//                        BucketName = DefaultBucketName,
//    					Key = clockFile.Name,
...
//			}
        ...
        public override bool Exists (RepoObject file)
        {
        ...
        {
            return null;//AllFiles.Where (rf => rf.RemoteMD5Hash == file.LocalMD5Hash && rf.AbsolutePath != file.AbsolutePath && !rf.Name.EndsWith("/") && rf.AsS3Object.Size>0).ToList<RepoObject> ();
        }
