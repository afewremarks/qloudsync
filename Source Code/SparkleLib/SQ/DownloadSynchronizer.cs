...
{
    public class DownloadSynchronizer :  SQ.Synchrony.Synchronizer
    {
        private static DownloadSynchronizer instance;
        private List<SQ.Repository.RemoteFile> PendingFiles;
        ...
        public override bool Synchronize ()
        {
            ...
            RemoteRepo.FilesChanged = new List<SQ.Repository.File>();
            SyncSize = 0;
            if (Initialize ()) {
      ...
      /*  public bool DownloadRemoteFiles ()
        ...
        {
			try{
	            foreach (SQ.Repository.File localFile in localFiles)
	            {
