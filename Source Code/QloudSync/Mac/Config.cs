...
namespace QloudSync {
    ...
    public class Config : XmlDocument {
        ...
        public string FoldersPath {
            get {
                if (GetConfigOption ("folders_path") != null)                      
                    ...
                    //return Path.Combine (HomePath, "QloudSync");
                    return Path.Combine (HomePath, "QloudSync");
            }
        }
        ...
        public SparkleUser User {
            get {
                XmlNode name_node = SelectSingleNode ("/qloudsync/user/name/text()");
                string name  = name_node.Value;

                XmlNode email_node = SelectSingleNode ("/qloudsync/user/email/text()");
                string email = email_node.Value;

                string pubkey_file_path = Path.Combine (
                    Path.GetDirectoryName (FullPath), "qloudsync." + email + ".key.pub");
        ...
        public List<string> Folders {
            get {
                ...
                foreach (XmlNode node_folder in SelectNodes ("/qloudsync/folder"))
        ...
        public void AddFolder (string name, string identifier, string url)
        {
            ...
            XmlNode node_root = SelectSingleNode ("/qloudsync");
        ...
        public void RemoveFolder (string name)
        {
            foreach (XmlNode node_folder in SelectNodes ("/qloudsync/folder")) {
                if (node_folder ["name"].InnerText.Equals (name))
                    SelectSingleNode ("/qloudsync").RemoveChild (node_folder);
            }
        ...
        public void RenameFolder (string identifier, string name)
        {
            XmlNode node_folder = SelectSingleNode (
                string.Format ("/qloudsync/folder[identifier=\"{0}\"]", identifier));
        ...
        {
            return SelectSingleNode (string.Format ("/qloudsync/folder[name=\"{0}\"]", name));
        }
        ...
        {
            XmlNode node = SelectSingleNode ("/qloudsync/" + name);
